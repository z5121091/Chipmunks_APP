...
export const getNextUnpackIndex = async (traceNo: string): Promise<number> => {
  try {
    // 直接从追溯码中提取序号
    // 格式：原始码 或 原始码-序号
    // 例如：ABC → 1, ABC-1 → 2, ABC-2 → 3
    
    const match = traceNo.match(/^(.+)-(\d+)$/);
    if (match) {
      // 如果追溯码已经是 XXX-N 格式，返回 N+1
...
export const getStatistics = async (): Promise<{
  ...
  try {
    ...
    // BUG 11, 12 修复：使用 YYYY/M/D 格式（不补零），与数据库存储格式一致
    const today = getISODateTime().slice(0, 10).replace(/-/g, '/');  // 转换为 YYYY/M/D 格式
    
    // 今日订单数
    const todayOrders = orders.filter(o => o.created_at.slice(0, 10) === today).length;
    
    // 今日物料数和数量
    const todayMaterialsList = materials.filter(m => m.scanned_at && m.scanned_at.slice(0, 10) === today);
...
export const parseWithRule = (
  ...
  fieldOrder.forEach((field, index) => {
    ...
    if (isCustomField(field)) {
      // 自定义字段：提取ID（去掉custom:前缀）并存入customFields
      const fieldId = getCustomFieldId(field);
      customFields[fieldId] = value;
    } else {
      // 标准字段存入standardFields
...
export const addUnpackRecord = async (record: {
  ...
  try {
    ...
    const remainingRecord: UnpackRecord = {
      ...
      productionDate: record.productionDate,
      traceNo: record.traceNo,
      new_traceNo: record.new_traceNo || '',  // 剩余标签也使用新追踪码
      sourceNo: record.sourceNo,
