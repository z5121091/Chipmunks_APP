...
export default function OrdersScreen() {
  ...
  // 筛选订单（根据仓库和时间）
  const filterOrdersByWarehouseAndTime = useCallback((allOrders: Order[]): Order[] => {
    let filtered = allOrders;
    
    // 按仓库筛选
    if (currentWarehouse) {
      filtered = filtered.filter(o => o.warehouse_name === currentWarehouse.name);
    }
  ...
  // 计算统计数据
  const calculateStats = useCallback(async (allOrders: Order[], allMaterials: MaterialRecord[]) => {
    // 筛选当前仓库的物料
    let warehouseMaterials = allMaterials;
    if (currentWarehouse) {
      warehouseMaterials = allMaterials.filter(m => m.warehouse_name === currentWarehouse.name);
    }
    ...
    const todayMaterialsList = warehouseMaterials.filter(m => m.scanned_at.slice(0, 10) === today);
    ...
    return {
      ...
      totalQuantity: warehouseMaterials.reduce((sum, m) => sum + (parseInt(m.quantity, 10) || 0), 0),
      todayOrders: todayOrderCount,
      todayMaterials: todayMaterialsList.length,
      todayQuantity: todayMaterialsList.reduce((sum, m) => sum + (parseInt(m.quantity, 10) || 0), 0),
      threeDaysOrders: threeDaysOrderCount,
      threeDaysMaterials: threeDaysMaterialsList.length,
      threeDaysQuantity: threeDaysMaterialsList.reduce((sum, m) => sum + (parseInt(m.quantity, 10) || 0), 0),
      sevenDaysOrders: sevenDaysOrderCount,
      sevenDaysMaterials: sevenDaysMaterialsList.length,
      sevenDaysQuantity: sevenDaysMaterialsList.reduce((sum, m) => sum + (parseInt(m.quantity, 10) || 0), 0),
    };
  }, [currentWarehouse]);
    ...
    try {
      ...
      if (currentExpandedId) {
        ...
        if (expandedOrder) {
          ...
          // 只显示当前仓库的物料
          const warehouseMaterials = currentWarehouse
            ? materials.filter(m => m.warehouse_name === currentWarehouse.name)
            : materials;
          setExpandedMaterials(warehouseMaterials);
        }
      }
    } catch (error) {
  ...
  const handleSearch = useCallback(async (text: string) => {
    ...
    if (searchType === 'batch') {
      ...
      try {
        ...
        // 只取当前仓库的物料
        const filteredMaterials = currentWarehouse
          ? materials.filter(m => m.warehouse_name === currentWarehouse.name)
          : materials;
        const orderNos = new Set(filteredMaterials.map(m => m.order_no));
  ...
  const handleTimeFilterChange = useCallback(async (filter: TimeFilterType) => {
    ...
    const getFilteredOrders = (allOrders: Order[], warehouse: Warehouse | null, timeFilterVal: TimeFilterType): Order[] => {
      ...
      // 按仓库筛选
      if (warehouse) {
        filtered = filtered.filter(o => o.warehouse_name === warehouse.name);
      }
    ...
    if (searchText.trim()) {
      ...
      if (searchType === 'batch') {
        // 按批次搜索（需要查询物料表）
        try {
          const materials = await searchMaterials({ batch: searchText });
          const filteredMaterials = currentWarehouse
            ? materials.filter(m => m.warehouse_name === currentWarehouse.name)
            : materials;
          const orderNos = new Set(filteredMaterials.map(m => m.order_no));
  ...
  const handleOpenMaterials = async (todayOnly: boolean = false) => {
    try {
      ...
      let materials = currentWarehouse
        ? allMaterials.filter(m => m.warehouse_name === currentWarehouse.name)
        : allMaterials;
  ...
  const handleOpenMaterialDetail = async (model: string) => {
    try {
      ...
      let filtered = currentWarehouse
        ? allMaterials.filter(m => m.warehouse_name === currentWarehouse.name && (m.model || '未知型号') === model)
      ...
      >
        ...
        <View style={{ paddingHorizontal: Spacing.md, marginBottom: Spacing.sm }}>
          ...
          <View style={styles.filterRow}>
            ...
            <TouchableOpacity
              style={[styles.searchTypeBtn, timeFilter === 'threeDays' && styles.searchTypeBtnActive]}
              onPress={() => handleTimeFilterChange('threeDays')}
            >
              <FontAwesome6 
                name="calendar-week" 
                size={12} 
            ...
            <TouchableOpacity
              style={[styles.searchTypeBtn, timeFilter === 'sevenDays' && styles.searchTypeBtnActive]}
              onPress={() => handleTimeFilterChange('sevenDays')}
            >
              <FontAwesome6 
                name="calendar-week" 
                size={12} 
          ...
          {timeFilter === 'all' ? (
            ...
            <View style={styles.statsCards}>
              ...
              <TouchableOpacity 
                ...
                onPress={() => handleOpenMaterials(timeFilter === 'today')}
              >
