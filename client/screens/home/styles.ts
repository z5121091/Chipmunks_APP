...
export const createStyles = (theme: Theme, screenWidth: number, screenHeight: number) => {
  ...
  // 布局配置 - 根据屏幕大小自适应
  const marginHorizontal = isSmallScreen ? 12 : Spacing['2xl'];  // 左右留白
  const marginTop = isSmallScreen ? 24 : Spacing['3xl'];          // 顶部留白
  const marginBottom = isSmallScreen ? 12 : Spacing['2xl'];     // 底部留白
